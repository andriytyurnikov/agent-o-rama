...
(defhook use-sente-query
  ...
  (let [state-path (into [:queries] query-key)
        ...
        page-is-visible? (use-page-visibility)
    ...
    (uix/use-effect
     (fn []
       (when (and should-refetch? connected? enabled? page-is-visible?)
         ...
         (fetch-data)))
     [should-refetch? connected? enabled? page-is-visible? fetch-data state-path])
...
(defhook use-paginated-query
  "A hook for paginated Sente queries that supports a 'load more' pattern.
   ...
   - :initial-pagination - Optional starting pagination cursor.
                           ...
                           (fn [reply]
                             ...
                             (if (:success reply)
                               (let [response-data (:data reply)
                                     ...
                                     current-data (or (get-in @state/app-state (into state-path [:data])) [])]
